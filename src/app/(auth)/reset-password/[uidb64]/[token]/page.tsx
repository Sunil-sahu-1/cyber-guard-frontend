...
export default function ResetPassword(){
  ...
  return <main className="space-bg flex min-h-screen items-center justify-center p-5">
      ...
      <Panel className="p-7 glow">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          <label className="block text-sm text-slate-400">
            ...
            <div className="relative mt-2">
              <Input type={show?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} className="pr-11" placeholder="Minimum 12 characters" required/>
          ...
          <label className="block text-sm text-slate-400">
            ...
            <div className="relative mt-2">
              <Input type={showConfirm?"text":"password"} value={confirm} onChange={e=>setConfirm(e.target.value)} className="pr-11" placeholder="Re-enter password" required/>
          ...
          {error&&<div className="rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-sm leading-6 text-red-200">{error}</div>}
        ...
        <div className="mt-6 text-center">
          <Link href="/login" className="text-sm text-cyan-300 hover:text-cyan-200">
            <ArrowLeft className="mr-1 inline h-4 w-4"/>Back to login
          </Link>
        </div>
      </Panel>
    </div>
