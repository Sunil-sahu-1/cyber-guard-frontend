      ...
      <Panel className="p-7 glow">
        ...
        <form onSubmit={submit} className="mt-7 space-y-4">
          <label className="block text-sm text-slate-400">
            User ID
            <Input value={id} onChange={e=>setId(e.target.value)} className="mt-2" placeholder="e.g. analyst01" required/>
          </label>

          <label className="block text-sm text-slate-400">
            Password
            <div className="relative mt-2">
              <Input type={show?"text":"password"} value={pw} onChange={e=>setPw(e.target.value)} placeholder="Minimum 12 characters" className="pr-11" required/>
        ...
        <div className="mt-6 border-t border-white/10 pt-5 text-center text-sm text-slate-500">
          ...
          <Link href="/register" className="text-cyan-300 hover:text-cyan-200">Create account</Link>
        </div>
      </Panel>
    </div>
  </main>
