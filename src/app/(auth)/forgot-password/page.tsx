    ...
    <div className="w-full max-w-md">
      ...
      <Panel className="p-7 glow">
        ...
        <p className="mt-1 text-sm leading-6 text-slate-500">
          Enter your registered email address. If an account exists, we will send a password reset link.
        </p>

        <form onSubmit={submit} className="mt-6 space-y-4">
          <label className="block text-sm text-slate-400">
            Email address
            <Input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="mt-2" placeholder="you@example.com" required/>
          </label>

          {message&&<div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-3 text-sm leading-6 text-emerald-200">{message}</div>}
