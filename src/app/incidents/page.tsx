...
import { Button, EmptyState, Panel, PageTitle, RiskBadge } from "@/components/ui";
import { listIncidents, resolveIncident } from "@/services/api/incidentApi";
  ...
  return (
    <ProtectedShell>
      <PageTitle
        ...
        description="Track high-severity events and the response actions created by the backend."
        action={
      ...
      <Panel className="overflow-hidden">
        <div className="overflow-x-auto">
          {items.length ? (
                    ...
                    <td className="px-5 py-4">
                      {!["RESOLVED", "CLOSED"].includes(i.status) && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg bg-emerald-400/10 px-2.5 py-1.5 text-xs text-emerald-300 ring-1 ring-emerald-300/15"
                        >
            ...
            <div className="p-8">
              <EmptyState text="No incidents in your queue." />
            </div>
          )}
        </div>
      </Panel>
