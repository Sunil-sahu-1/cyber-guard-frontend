...
export default function PhishingUrlPage() {
  ...
  async function run() {
    ...
    setLoading(true);
    try {
      setResult(await analyzeUrl(url.trim()));
    } catch (e) {
      setErr(e instanceof Error ? e.message : "URL analysis failed");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <ProtectedShell>
      <PageTitle
        title="URL Analysis"
        description="Analyze suspicious URLs using the backend phishing detection engines."
      />
      <div className="grid gap-5 xl:grid-cols-[1fr_.95fr]">
        ...
        {result ? (
          ...
          <Empty text="Submit a URL to see the backend model output." />
        )}
      </div>
    </ProtectedShell>
  );
