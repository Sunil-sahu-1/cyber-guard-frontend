...
export default function Anomalies() {
  ...
  async function run() {
    setBusy(true);
    try {
      const response = kind === "login" ? await analyzeLogin(form) : await analyzeBehaviour(form);
      ...
      setResult({
        ...
        explanation:
          error instanceof Error ? error.message : "The anomaly analysis could not be completed.",
      });
    } finally {
      setBusy(false);
    }
      ...
      <Panel className="mt-5 p-6">
        ...
        <div className="mt-4 space-y-2">
          {items.slice(0, 8).map((anomaly) => (
            ...
            >
              <div>
                <div className="text-sm text-slate-300">{humanize(anomaly.anomaly_type)}</div>
                <div className="text-xs text-slate-600">
                  #{anomaly.id} · {new Date(anomaly.detected_at).toLocaleString()}
                </div>
              </div>
  ...
  if (!result) {
    return (
      <Panel className="grid min-h-[500px] place-items-center p-6">
        <EmptyState text="Run an anomaly analysis to see a clear risk summary, detected signals and recommended actions." />
      </Panel>
    );
  }
    ...
    <Panel className="p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {!isError && <div className="mt-1 text-sm text-slate-500">{prediction}</div>}
        </div>
        ...
        <>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-white/[.025] p-4">
              ...
              <div className="mt-1 text-lg font-semibold">{score.toFixed(2)} / 100</div>
            </div>

            <div className="rounded-xl border border-white/10 bg-white/[.025] p-4">
              <div className="text-xs text-slate-600">Confidence</div>
              <div className="mt-1 text-lg font-semibold">
                {confidence === null ? "Not provided" : confidence.toFixed(0) + "%"}
              </div>
            </div>
          </div>
          ...
          <section className="mt-5">
            ...
            {actions.length ? (
              ...
              <p className="mt-2 text-sm text-slate-600">No additional actions were returned.</p>
            )}
          </section>
        </>
      )}
