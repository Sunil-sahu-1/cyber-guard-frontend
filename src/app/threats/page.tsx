...
import { EmptyState, Input, PageTitle, RiskBadge, Panel, Button } from "@/components/ui";
...
export default function Threats() {
  ...
  const filtered = items.filter((t) =>
    (t.threat_type + " " + t.source_type + " " + t.severity + " " + t.status)
      .toLowerCase()
  ...
  return (
    <ProtectedShell>
      <PageTitle
        ...
        description="Review AI-detected threats, evidence, model analyses, and status."
        action={
      ...
      <Panel className="overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {loading ? (
              ...
              <tbody>
                {filtered.map((t) => (
                  <tr key={t.id} className="border-b border-white/5 hover:bg-white/[.025]">
                    ...
                    <td className="px-5 py-4 text-slate-400">{t.status.replaceAll("_", " ")}</td>
            ...
            <div className="p-8">
              <EmptyState text="No matching threats found." />
            </div>
          )}
        </div>
      </Panel>
