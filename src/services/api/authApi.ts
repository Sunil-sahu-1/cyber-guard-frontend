...
export async function requestPasswordReset(email:string){return apiFetch<{message:string}>("/auth/password-reset/",{method:"POST",body:JSON.stringify({email})})}
export async function confirmPasswordReset(uidb64:string,token:string,password:string,password_confirm:string){return apiFetch<{message:string}>(`/auth/password-reset/confirm/${uidb64}/${token}/`,{method:"POST",body:JSON.stringify({password,password_confirm})})}
